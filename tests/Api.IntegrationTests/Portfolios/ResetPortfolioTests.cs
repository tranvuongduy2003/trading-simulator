...
public sealed class ResetPortfolioTests(IntegrationTestFixture fixture)
{
    ...
    public async Task GetResetEligibility_WithoutPriorReset_IsEligible()
    {
        ...
        var body = await eligibilityResponse.Content.ReadFromJsonAsync<PortfolioResetEligibilityResponse>(JsonOptions);
    ...
    public async Task ResetPortfolio_WhenCooldownActive_Returns422_WithNextEligibleAt()
    {
        ...
        var resetCountBefore = await PortfolioResetTestHelpers.CountPortfolioResetsAsync(
            fixture,
            registration.UserId);
        ...
        var nextEligibleAt = DateTimeOffset.Parse(nextEligibleAtRaw!, CultureInfo.InvariantCulture);
        nextEligibleAt.Should().BeCloseTo(expectedNextEligibleAt, TimeSpan.FromMinutes(1));

        var resetCountAfter = await PortfolioResetTestHelpers.CountPortfolioResetsAsync(
            fixture,
            registration.UserId);
    ...
    public async Task ResetPortfolio_FirstReset_SucceedsAndReturnsNextEligibleAt()
    {
        ...
        using var eligibilityResponse = await client.GetAsync("/api/portfolio/reset/eligibility");
        eligibilityResponse.StatusCode.Should().Be(HttpStatusCode.OK);
        var eligibility = await eligibilityResponse.Content.ReadFromJsonAsync<PortfolioResetEligibilityResponse>(JsonOptions);
    ...
    public async Task ResetPortfolio_WhenCooldownActive_DoesNotMutateState()
    {
        ...
        var openOrders = await openOrdersResponse.Content.ReadFromJsonAsync<List<TradingSimulator.Contracts.Orders.OpenOrderDto>>(JsonOptions);
