...
Start the MCP server for agentic tools (e.g. Cursor root `.mcp.json`).

```bash
aspire agent mcp [options]

# Options:
#   --dashboard-url <url>   Standalone Aspire Dashboard base URL (optional)
#   --api-key <key>         Dashboard API key when required
```

This repo's [`.mcp.json`](../../../../.mcp.json) invokes this command via `"args": ["agent", "mcp"]`.
...
#   start   Start the MCP server (VS Code / Copilot config)
```
