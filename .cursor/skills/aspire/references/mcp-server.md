...
Repo root [`.mcp.json`](../../../../.mcp.json) wires the Aspire MCP server for Cursor agents:
...
| Detail | Value |
|---|---|
| Server id in Cursor | `aspire` |
| CLI entry point | `aspire agent mcp` (STDIO) |
| Prerequisites | Aspire CLI on `PATH` (`aspire --version`) |
| Runtime tools | AppHost running (`aspire run`) or `--dashboard-url` for a standalone dashboard |

**vs VS Code Copilot:** `aspire mcp init` typically writes `"args": ["mcp", "start"]`. Both start the same MCP tool surface; this repo uses the **agent** subcommand for Cursor.

**When agents should call MCP**

1. AppHost / resource questions while developing → `list_resources`, health, endpoints
2. Failures → `list_console_logs`, `list_structured_logs`, `list_traces`
3. Integration wiring → `list_integrations`, `get_integration_docs`
4. aspire.dev how-to (CLI 13.2+) → `search_docs`, `get_doc`

Project workflow and topology: `.cursor/rules/aspire.mdc`.
...
### Cursor (manual / this repo)

Root [`.mcp.json`](../../../../.mcp.json) uses the agent subcommand:
...
Run `aspire agent mcp --help` for `--dashboard-url` and `--api-key` when not using the default in-process dashboard.
