...
metadata:
    github-path: skills/aspire
    github-ref: refs/heads/main
    github-repo: https://github.com/github/awesome-copilot
    github-tree-sha: afafff53a657a959d83109b5da558426126eec74
name: aspire
---
...
### This repository (trading-simulator)

| Item | Location |
|---|---|
| Cursor rule (topology, workflow) | `.cursor/rules/aspire.mdc` |
| MCP server config | [`.mcp.json`](../../../.mcp.json) — server id `aspire`, command `aspire agent mcp` |
| AppHost (topology source of truth) | `src/**/AppHost/` when present |
| Technical spec | `docs/TECHNICAL.md` §12 |

When debugging the local stack, use the **aspire** MCP tools (resource list, logs, traces) if MCP is enabled — do not infer ports or health from memory. For AppHost wiring in this repo, follow `aspire.mdc` and `aspire-configuration` skill.
...
The Aspire team ships an **MCP server** that provides documentation and runtime tools inside your AI assistant. See [MCP Server](references/mcp-server.md) for setup details.

### Cursor in this repo

[`.mcp.json`](../../../.mcp.json) registers the server as **`aspire`** (STDIO):
...
This is the **agent** entry point (`aspire agent mcp`), not the older VS Code snippet `aspire mcp start`. Optional: `--dashboard-url` / `--api-key` when attaching to a standalone dashboard.

Use MCP **before** manual log diving when the stack is running: `list_resources`, `list_console_logs`, `list_traces`, `execute_resource_command`. For package/docs questions without a running AppHost: `list_integrations`, `get_integration_docs`, and on CLI 13.2+: `search_docs`, `get_doc`.
...
| `aspire mcp init` | Configure MCP for AI assistants (interactive) | Stable |
| `aspire mcp start` | Start MCP server (VS Code / legacy config) | Stable |
| `aspire agent mcp` | Start MCP server (Cursor `.mcp.json` in this repo) | Stable |
